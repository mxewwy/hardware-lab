...
public final class ModBlocks {
    ...
    public static final Block EIGHT_BIT_REGISTER = register(
            "eight_bit_register",
            EightBitRegisterBlock::new,
            BlockBehaviour.Properties.of().strength(1.5F).sound(SoundType.METAL)
    );

    public static final Block RAM_256 = register(
            "ram_256",
            Ram256Block::new,
            BlockBehaviour.Properties.of().strength(2.0F).sound(SoundType.METAL)
    );

    public static final Block ROM_256 = register(
            "rom_256",
            Rom256Block::new,
            BlockBehaviour.Properties.of().strength(2.0F).sound(SoundType.METAL)
    );

    public static final Block SEVEN_SEGMENT_DISPLAY = register(
            "seven_segment_display",
            SevenSegmentDisplayBlock::new,
            BlockBehaviour.Properties.of().strength(1.0F).sound(SoundType.METAL).noOcclusion()
    );

    public static final Block LED_MATRIX = register(
            "led_matrix",
            LedMatrixBlock::new,
            BlockBehaviour.Properties.of().strength(1.0F).sound(SoundType.METAL).noOcclusion()
    );

    public static final Block CPU = register(
            "cpu",
            CpuBlock::new,
            BlockBehaviour.Properties.of().strength(2.5F).sound(SoundType.METAL)
    );

    public static final Block FPGA = register(
            "fpga",
            FpgaBlock::new,
            BlockBehaviour.Properties.of().strength(2.0F).sound(SoundType.METAL)
    );
